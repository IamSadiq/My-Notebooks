...
positive_reviews = BeautifulSoup(open('data/positive.review.txt').read(), features="html5lib")
positive_reviews = positive_reviews.findAll('review_text')

negative_reviews = BeautifulSoup(open('data/negative.review.txt').read(), features="html5lib")
...
def tokens_2_vector(tokens):
    ...
    x = np.zeros([11292]) # explicitly setting the number of features to the same number of features used in training
    for t in tokens:
        i = word_index_mapper[t]
        x[i] += 1
    x = x / x.sum()
    return x

def predict_using_LogReg(phrase):
    tokenized_chat = tokener(phrase)
    current_index = 0

    for token in tokenized_chat:
        if token not in word_index_mapper:
            word_index_mapper[token] = current_index
            current_index += 1

    # vectorise the chat tokens
    vectorised_chat = tokens_2_vector(tokenized_chat)
    
    print(vectorised_chat)

    x_data = np.zeros((1, 11292))
    x_data[0,:] = vectorised_chat

    # Reshape your data either using array.reshape(-1, 1) if your data has a single feature
    # OR array.reshape(1, -1) if it contains a single sample.

    # since we are making inference on single samples in this case
    x_data = x_data.reshape(1, -1)
    
    print(x_data)

    # load the model
#     model_path = './models/sa_logreg_model.sav'
#     model_path = './models/LogRegModel.sav'
